...
Tokens are stored in the container at:

| File | Purpose |
|------|---------|
| `~/.config/hytale/tokens/oauth_tokens.json` | OAuth access & refresh tokens |
| `~/.config/hytale/tokens/session_tokens.json` | Game session tokens |
| `~/.config/hytale/tokens/profiles.json` | Cached user profiles |
| `~/.config/hytale/tokens/selected_profile.json` | Currently selected profile |
| `/server/.cache/.hytale-downloader-credentials.json` | Credentials for downloader |

<Callout type="info">
Persist the `hytale-home` volume to keep tokens across container restarts.
</Callout>
...
Ensure the `hytale-home` volume is persisted:
```yaml
volumes:
